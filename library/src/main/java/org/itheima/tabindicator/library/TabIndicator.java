...
public class TabIndicator
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mTabContainer.setOrientation(LinearLayoutCompat.HORIZONTAL);
    ...
    private float[] getCurrentBounds() {
        ...
        if (left == right)
            throw new IllegalStateException("please override getPageTitle(int position) or invoke setTitles(List<CharSequence> titles)");
        ...
        if (isWeight) {
            ...
            int textWidth = getTextWH(currentTab, text)[0];
